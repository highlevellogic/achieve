...
process.on("uncaughtException",function (error) {
    if (process.send) process.send({event:"uncaught",error:String(error)});
});
process.on("unhandledRejection",function (error) {
    if (process.send) process.send({event:"unhandled",error:String(error)});
});

const port=Number(process.env.ACHIEVE_BUFFERED_ABORT_PORT);
const protocol=process.env.ACHIEVE_BUFFERED_ABORT_PROTOCOL;
let server;
if (protocol === "http") {
    ...
    throw new Error("Unknown buffered-abort fixture protocol.");
}
